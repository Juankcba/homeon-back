...
build-ai: ## Rebuild the AI engine container
...
health: ## Check health of all services
	...
	@echo "$(CYAN)AI Engine:$(RESET)"
	@docker compose ps ai-engine --format '{{.Status}}' 2>/dev/null | grep -qi "up" && echo " ✓ OK" || echo " ✗ DOWN"
...
deploy: ## Pull latest code + rebuild + restart (backend + AI)
	@echo "$(CYAN)Pulling latest code...$(RESET)"
	git pull origin main
	@echo "$(CYAN)Building backend...$(RESET)"
	docker compose build backend
	@echo "$(CYAN)Building AI engine...$(RESET)"
	docker compose build ai-engine
	@echo "$(CYAN)Restarting all services...$(RESET)"
	docker compose up -d
	@echo "$(GREEN)Deploy complete!$(RESET)"

deploy-backend: ## Rebuild and restart only the backend
	docker compose build backend
	docker compose up -d backend

deploy-ai: ## Rebuild and restart only the AI engine
	docker compose build ai-engine
